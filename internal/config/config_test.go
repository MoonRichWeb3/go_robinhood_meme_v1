...
func TestLoadDefaultsAndValidation(t *testing.T) {
	keys := []string{
		...
		"RH_TRACE_RPC_URL", "RH_TRACE_TIMEOUT_MS", "RH_RPC_ENDPOINTS_FILE", "RH_ALCHEMY_RPC_URL",
		...
		"RH_EVENT_PURGE_BATCH", "RH_EVENT_PURGE_SLEEP_MS", "RH_EVENT_PURGE_MAX_PER_RUN", "RH_SKIP_HISTORY_LAG",
	}
	for _, key := range keys {
		t.Setenv(key, "")
	}
	t.Setenv("RH_RPC_ENDPOINTS_FILE", filepath.Join(t.TempDir(), "missing-rpcs.json"))
	...
	if got.ChainID != 4663 || got.TraceRPCURL != "" || got.TraceTimeoutMS != 8000 ||
		...
		len(got.RPCEndpoints) != 1 || got.RPCEndpoints[0].QPS != 1 || got.SkipHistoryLag != 100 {
