...
| 问题 | 结论 |
| --- | --- |
| 能否监听？ | 能。优先扫 Pons `TokenLaunched`/`CurveBuy`、o1 `Launched`/`Swap`，以及 `PoolManager.Swap`。 |
...
1. **发现**：Pons V2 `TokenLaunched` / `launchAndBuy` + o1 股票工厂 `Launched`（可选再加 o1 加密工厂、pools.trade）。
2. **交易**：Pons 未毕业走曲线 `buy`；Pons 毕业盘与 o1 一律 Universal Router（hooks 分别是 Pons meme hook / o1 LaunchHook）。o1 股票盘的 quote 是股票代币，不是 ETH。
